...
const LinkFeed = props => {
  const { classes } = props
  const history = useHistory();
  const [links, setLinks] = useState([]);

  useEffect(() => {
    axios.get(`/links/${props.match.params.channelName}`)
      .then(res => {
        let orderedLinks = res.data.sort((a, b) => b.timestamp - a.timestamp);
...
export default withStyles(styles)(LinkFeed);